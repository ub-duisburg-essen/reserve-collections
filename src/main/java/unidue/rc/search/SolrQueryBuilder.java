...
public class SolrQueryBuilder {
    ...
    public SolrQuery build() {
        ...
        if (queryStringBuilder.length() == 0)
            query.setQuery(Character.toString(WILDCARD_SIGN));
    ...
    private String trimSearchValue(String value) {
        ...
        IntStream.range(0, value.length())
                .forEach(i -> b.append(getEscapedString(value.charAt(i))));
        return b.toString();
    }

    private String getEscapedString(char c) {
        return c == WILDCARD_SIGN || c == DIVIDER
               ? StringUtils.EMPTY
